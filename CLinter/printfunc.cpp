    ...
    while (!lx_peek_stmt_sep(lx)) {
        ...
        if (lx->cur.type == T_IDENT) {
            ...
            if (_stricmp(id, "TAB") == 0) {
                ...
                /* If the next non-space is ':' or '\' or end, stop PRINT here. */
                if (lx_peek_stmt_sep(lx)) break;
        ...
        if (lx->cur.type != T_COMMA && lx->cur.type != T_SEMI) {
            g_print_col = 1; // Reset at newline
        }
